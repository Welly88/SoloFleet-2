...
## 🔴 ThemeProvider — NEVER Modify

**NEVER output or modify `components/ThemeProvider.tsx`.** The ThemeProvider handles global CSS variable injection for Modal support — editing it will break dark mode in modals. It is a scaffold file that must remain untouched.

## 🔴 Modal — No Hardcoded Colors

When using `<Modal>`, **NEVER hardcode `backgroundColor`** on the modal content View. Use semantic classes:
```tsx
// ✅ CORRECT — semantic bg-card adapts to theme
<Modal transparent animationType="fade">
  <View className="flex-1 justify-center items-center" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
    <View className="bg-card rounded-2xl p-6 w-[90%]">
      <Text className="text-card-foreground">Content</Text>
    </View>
  </View>
</Modal>

// ❌ WRONG — hardcoded white/dark breaks the other theme
<View style={{ backgroundColor: '#ffffff' }}>
<View style={{ backgroundColor: '#1a1a1a' }}>
```
The only acceptable inline `backgroundColor` in a Modal is the semi-transparent overlay (`rgba(0,0,0,0.5)`). All other backgrounds must use `className` with semantic colors (`bg-card`, `bg-background`).
